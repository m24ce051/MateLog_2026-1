  ...
  return (
    <div className="lesson-detail-container">
      <header className="lesson-detail-header">
        ...
        <h1>{lesson.titulo}</h1>
        {/* FIX: NO mostrar descripción en esta pantalla */}
      </header>
        ...
        {lesson.temas && lesson.temas.length > 0 ? (
          <div className="topics-grid">
            {lesson.temas.map((topic) => {
              ...
              // Calcular progreso de contenido (sin contar EJEMPLO_EXTRA)
              const contenidoTotal = topic.contenidos_count || 0;
              const contenidoCompletado = topic.progreso?.contenido_completado || 0;
              const porcentajeContenido = contenidoTotal > 0
                ? (contenidoCompletado / contenidoTotal) * 100
                : 0;
                ...
                >
                  ...
                  <div className="progress-section">
                    <div className="progress-label">
                      <span>Progreso de contenido</span>
                      <span className="progress-percentage">{Math.round(porcentajeContenido)}%</span>
                    </div>
                    <div className="progress-bar-container">
                      <div
                        className="progress-bar-fill"
                        style={{ width: `${porcentajeContenido}%` }}
                      />
                    </div>
                  </div>
                 
                  {/* Mensaje cuando no se han completado ejercicios */}
                  {porcentajeAcierto === 0 && (
                    <div className="exercises-pending-message">
                      <span>⏳ Ejercicios pendientes</span>
                    </div>
                  )}
                 

                  {/* Calificación de ejercicios (solo si ha hecho ejercicios) */}
                  {porcentajeAcierto > 0 && (
                    <div className="score-section">
                      <div className="score-label">
                        <span>Calificación de ejercicios</span>
                        <span className="score-percentage">{Math.round(porcentajeAcierto)}%</span>
                      </div>
                      <div className="score-bar-container">
                        <div
                          className={`score-bar-fill ${porcentajeAcierto >= 80 ? 'approved' : 'not-approved'}`}
